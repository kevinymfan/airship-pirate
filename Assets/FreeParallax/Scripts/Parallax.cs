...
public delegate void ParallaxElementRepositionLogicFunction(Parallax p, ParallaxElement element, float amount, GameObject obj, Renderer r);
...
public enum ParallaxPositionMode
{
...
/// </summary>
[Serializable]
public class ParallaxElementRepositionLogic
{
    ...
    public ParallaxPositionMode PositionMode = ParallaxPositionMode.WrapAnchorBottom;
...
public class ParallaxElement
{
    ...
    public ParallaxElementRepositionLogic RepositionLogic;

    [HideInInspector]
    public ParallaxElementRepositionLogicFunction RepositionLogicFunction;

    public void SetupState(Parallax p, Camera c, int index)
    {
        // add a second object if we need one to wrap properly
        if (RepositionLogic.PositionMode != ParallaxPositionMode.IndividualStartOffScreen &&
            RepositionLogic.PositionMode != ParallaxPositionMode.IndividualStartOnScreen &&
            GameObjects.Count == 1)
        {
    ...
    public void SetupPosition(Parallax p, Camera c, int index)
    {
        ...
        for (int i = 0; i < GameObjects.Count; i++)
        {
            ...
            {
                float x, y;
                x = (RepositionLogic.PositionMode == ParallaxPositionMode.IndividualStartOnScreen ? r.bounds.min.x : 0);
                y = (RepositionLogic.PositionMode == ParallaxPositionMode.IndividualStartOnScreen ? r.bounds.min.y : start + r.bounds.size.y);
            ...
            else
            {
                ...
                if (RepositionLogic.PositionMode == ParallaxPositionMode.WrapAnchorTop)
                {
                ...
                else if (RepositionLogic.PositionMode == ParallaxPositionMode.WrapAnchorBottom)
                {
                    if (p.IsHorizontal)
                    {
    ...
    public void Update(Parallax p, float t, Camera c)
    {
        ...
        bool wrap = RepositionLogic.PositionMode != ParallaxPositionMode.IndividualStartOffScreen &&
            RepositionLogic.PositionMode != ParallaxPositionMode.IndividualStartOnScreen;
